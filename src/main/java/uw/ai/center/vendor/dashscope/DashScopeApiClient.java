 ...
 * <p>
 * 使用 uw-base 的 JsonInterfaceHelper（OkHttp 封装）进行 HTTP 通信。
 * <p>
 * 注意：DashScope 有两套 API 地址：
 * - OpenAI 兼容模式：https://dashscope.aliyuncs.com/compatible-mode/v1（仅支持 Chat/Embedding）
 * - 原生 API：https://dashscope.aliyuncs.com/api/v1（支持图片生成、语音识别等全部能力）
 * <p>
 * 本客户端统一使用原生 API 地址，如果配置的 apiUrl 是兼容模式地址，会自动转换为原生 API 地址。
 */
public class DashScopeApiClient {
     ...
     * @return 图片 URL
     */
    public static String generateImage(String baseUrl, String apiKey, String model, String prompt, Map<String, Object> params) {
        // 自动转换 URL
        ...
        try {
            ...
            if ("200".equals(statusCode) && responseJson.has("output")) {
                ...
                if ("SUCCEEDED".equals(outputNode.path("task_status").asText())) {
                    return extractImageUrl(outputNode);
                }
            }
            ...
            for (int i = 0; i < maxRetries; i++) {
                ...
                switch (taskStatus) {
                    case "SUCCEEDED":
                        return extractImageUrl(queryJson.path("output"));
                    case "FAILED":
     ...
     * 从任务输出中提取图片 URL。
     */
    private static String extractImageUrl(JsonNode outputNode) {
        JsonNode resultsNode = outputNode.path("results");
        if (resultsNode.isArray() && !resultsNode.isEmpty()) {
            String url = resultsNode.get(0).path("url").asText();
            if (!url.isEmpty()) {
                return url;
            }
            // 有些模型返回 b64_image
            String b64Image = resultsNode.get(0).path("b64_image").asText();
            if (!b64Image.isEmpty()) {

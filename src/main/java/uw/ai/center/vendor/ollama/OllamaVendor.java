...
public class OllamaVendor implements AiVendor {
    ...
    public List<ConfigParam> vendorParam() {
        return List.of(
                ...
                new ConfigParam( "main-gpu", "0", TypeConfigData.INT.getValue(), "main-gpu", "当使用多个 GPU 时，此选项控制使用哪个 GPU 来处理小张量，因为将计算拆分到所有 GPU 上的开销不值得。所指的 GPU 将使用略多的 VRAM 来存储临时结果的临时缓冲区。" ),
                ...
                new ConfigParam( "use-mmap", "true", TypeConfigData.BOOLEAN.getValue(), "use-mmap", "默认情况下，模型会被映射到内存中，这使得系统可以根据需要只加载模型的必要部分。然而，如果模型的大小超过了你的总 RAM 量，或者你的系统可用内存不足，使用 mmap 可能会增加页面换出的风险，从而负面影响性能。禁用 mmap 会导致加载时间变慢，但如果不用 mlock，可能会减少页面换出。请注意，如果模型的大小超过了总 RAM 量，关闭 mmap 将阻止模型加载。" ),
                new ConfigParam( "use-mlock", "false", TypeConfigData.BOOLEAN.getValue(), "use-mlock", "将模型锁定在内存中，防止在内存映射时将其交换出去。这可以提高性能，但会牺牲一些内存映射的优势，因为它需要更多的 RAM 来运行，并且可能在模型加载到 RAM 时减慢加载时间。" ),
                new ConfigParam( "num-thread", "0", TypeConfigData.INT.getValue(), "num-thread", "设置在计算过程中使用的线程数量。默认情况下，Ollama 会检测此值以获得最佳性能。建议将此值设置为系统中的物理 CPU 核心数（而不是逻辑核心数）。0 = 由运行时决定" ),
                ...
                new ConfigParam( "proxy-tool-calls", "false", TypeConfigData.BOOLEAN.getValue(), "proxy-tool-calls", "如果为真，则将不会处理函数调用，而是将它们代理给客户端。然后是客户端的责任来处理函数调用，将它们分发到适当的函数，并返回结果。如果为假（默认值），则 Spring AI 将内部处理函数调用。仅适用于具有函数调用支持的聊天模型" )
        );
    }

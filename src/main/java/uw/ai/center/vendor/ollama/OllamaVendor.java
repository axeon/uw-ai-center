...
public class OllamaVendor implements AiVendor {
    ...
    public List<ConfigParam> modelParam() {
        return List.of(
                ...
                new ConfigParam("use-mmap","true","boolean","use-mmap","默认情况下，模型会被映射到内存中，这使得系统可以根据需要只加载模型的必要部分。然而，如果模型的大小超过了你的总 RAM 量，或者你的系统可用内存不足，使用 mmap 可能会增加页面换出的风险，从而负面影响性能。禁用 mmap 会导致加载时间变慢，但如果不用 mlock，可能会减少页面换出。请注意，如果模型的大小超过了总 RAM 量，关闭 mmap 将阻止模型加载。"),
                ...
                new ConfigParam("频率抑制","0","int","频率抑制","-"),
                new ConfigParam("米罗斯塔姆","0","int","米罗斯塔姆","启用米罗斯塔姆采样以控制困惑度。(默认: 0, 0 = 禁用, 1 = 米罗斯塔姆, 2 = 米罗斯塔姆 2.0)"),
                new ConfigParam("米罗斯塔姆 Tau","5","int","米罗斯塔姆 Tau","控制输出的连贯性和多样性之间的平衡。较低的值会导致更专注和连贯的文本。"),
                ...
                new ConfigParam("proxy-tool-calls","false","boolean","proxy-tool-calls","如果为真，则 Spring AI 将不会处理函数调用，而是将它们代理给客户端。然后是客户端的责任来处理函数调用，将它们分发到适当的函数，并返回结果。如果为假（默认值），则 Spring AI 将内部处理函数调用。仅适用于具有函数调用支持的聊天模型")
        );
    }

...
public class OpenAiVendor implements AiVendor {
    ...
    public List<ConfigParam> vendorParam() {
        return List.of(
                ...
                new ConfigParam( "temperature", "0.8", TypeConfigData.FLOAT.getValue(), "temperature",
                        "要使用的采样温度，用于控制生成的完成项的明显创造性。较高的值将使输出更具随机性，而较低的值将使结果更加集中和确定。不建议为相同的completions请求修改 temperature 和top_p，因为这两个设置的交互很难预测。" ),
                new ConfigParam( "frequency-penalty", "0.0f", TypeConfigData.FLOAT.getValue(), "frequency-penalty", "介于 -2.0 和 2.0 " +
                ...
                new ConfigParam( "output-modalities", "", TypeConfigData.STRING.getValue(), "output-modalities", "您希望模型为此请求生成的 Output types。大多数模型都能够生成文本，这是默认设置。gpt-4o-audio" +
                        "-preview模型也可用于生成音频。要请求此模型同时生成文本和音频响应，您可以使用：text，audio。不支持流式传输。" ),
                new ConfigParam( "output-audio", "", TypeConfigData.STRING.getValue(), "output-audio", "音频生成的音频参数。使用output-modalities：audio请求音频输出时是必需的。需要gpt-4o-audio-preview" +
                        "模型，并且不支持流式完成。" ),
                ...
                new ConfigParam( "top-p", "", TypeConfigData.FLOAT.getValue(), "top-p", "使用温度进行采样的替代方法，称为核采样，其中模型考虑具有top_p概率质量的标记的结果。所以 0.1 意味着只考虑包含前 10% 概率质量的 " +
                        "token。我们通常建议更改此温度或温度，但不能同时更改两者。" ),
                new ConfigParam( "tools", "", TypeConfigData.SET_STRING.getValue(), "tools", "模型可以调用的工具列表。目前，仅支持将函数作为工具。使用此函数可提供模型可能为其生成 JSON 输入的函数列表。" ),
                new ConfigParam( "tool-choice", "", TypeConfigData.STRING.getValue(), "tool-choice", "控制模型调用哪个 （如果有） 函数。none表示模型不会调用函数，而是生成一条消息。auto" +
